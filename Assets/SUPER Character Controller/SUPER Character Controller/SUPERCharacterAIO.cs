...
namespace SUPERCharacter {
    ...
    public class SUPERCharacterAIO : MonoBehaviour {
        #region Variables
        //public Transform respawnPoint;// FLY TO BASE SYSTEM
        //public float flyHeight = 4f;
        //public float flyDuration = 1.2f;
        //private bool isFlyingToBase = false;
        //private Vector3 flyStartPos;
        //private float flyTimer ;
        public bool isFlyingToBase = false;
        ...
        private float flyTimer;
        //public float respawnFreezeTime = 1.2f;
        public bool controllerPaused = false;
        #region Hammer Settings
        [Header("Hammer Settings")]
        public GameObject hammer3D;
       

        public float hammerHideDelay = 5f;
    ...
    public Key interactKey = Key.E;
        ...
        void Update() {
            ...
            if (isFlyingToBase)
            {
                flyTimer += Time.deltaTime / flyDuration;
                float t = Mathf.Clamp01(flyTimer);

                Vector3 horizontal = Vector3.Lerp(flyStart, flyTarget.position, t);
                float height = Mathf.Sin(t * Mathf.PI) * flyHeight;

                transform.position = new Vector3(
                    horizontal.x,
                    flyTarget.position.y + height,
                    horizontal.z
                );
...
#else
                //camera
                MouseXY.x = ControlFreak2.CF2Input.GetAxis("Mouse Y");
                MouseXY.y = ControlFreak2.CF2Input.GetAxis("Mouse X");
                mouseScrollWheel = ControlFreak2.CF2Input.GetAxis("Mouse ScrollWheel");
                perspecTog = ControlFreak2.CF2Input.GetKeyDown(perspectiveSwitchingKey_L);
                interactInput = ControlFreak2.CF2Input.GetKeyDown(interactKey_L);
                //movement

                jumpInput_Momentary = ControlFreak2.CF2Input.GetKey(jumpKey_L);
                jumpInput_FrameOf = ControlFreak2.CF2Input.GetKeyDown(jumpKey_L);
                crouchInput_Momentary = ControlFreak2.CF2Input.GetKey(crouchKey_L);
                crouchInput_FrameOf = ControlFreak2.CF2Input.GetKeyDown(crouchKey_L);
                sprintInput_Momentary = ControlFreak2.CF2Input.GetKey(sprintKey_L);
                sprintInput_FrameOf = ControlFreak2.CF2Input.GetKeyDown(sprintKey_L);
                slideInput_Momentary = ControlFreak2.CF2Input.GetKey(slideKey_L);
                slideInput_FrameOf = ControlFreak2.CF2Input.GetKeyDown(slideKey_L);
#if SAIO_ENABLE_PARKOUR

            vaultInput = ControlFreak2.CF2Input.GetKeyDown(VaultKey_L);
#endif
                MovInput = Vector2.up * ControlFreak2.CF2Input.GetAxisRaw("Vertical") + Vector2.right * ControlFreak2.CF2Input.GetAxisRaw("Horizontal");
#endif
                #endregion
        ...
        //    // Mark that button is being held
        //    isAttackButtonHeld = true;

        //    // Show hammer while holding
        //    if (hammer3D != null)
        //        hammer3D.SetActive(true);

        //    // Play attack animation
        //    _3rdPersonCharacterAnimator.SetTrigger("Attack");

        //    // Delay hit check
        //    Invoke(nameof(TryHitAI), 0.15f);
        //}


        //// Called when UI button is released
        //public void AttackButtonReleased()
        //{
        //    isAttackButtonHeld = false;

        //    // Cancel any previous hide invokes
        //    CancelInvoke(nameof(HideHammer));

        //    // Start timer to hide hammer
        //    Invoke(nameof(HideHammer), hammerHideDelay);
        //}


        //private void HideHammer()
        //{
        //    // If button pressed again, do NOT hide
        //    if (isAttackButtonHeld)
        //        return;

        //    if (hammer3D != null)
        //        hammer3D.SetActive(false);
        //}
        public void Attack()
        {
            isAttackButtonHeld = true;

            // Show any current weapon
            ...
            // Restart hide timer
            CancelInvoke(nameof(HideCurrentWeapon));
            Invoke(nameof(HideCurrentWeapon), weaponHolder.hideDelay);
        }
        ...
        {
            // If still pressing attack → do NOT hide
            if (isAttackButtonHeld)
                return;

            // If no weapon equipped → nothing to hide
            if (weaponHolder.currentWeapon == null)
                return;
        ...
        // AI Hit Logic stays SAME
        private void TryHitAI()
        {
            ...
            {
                AIAnimationController ai = hit.GetComponent<AIAnimationController>();

                if (ai != null)
                {
                    Debug.Log("HIT AI! Making it fall...");
                    ai.Fall();
                }
            }
        }
        // -------------------------
        // GET HIT BY AI (PLAYER FALL)
        // -------------------------
        //public void GetHitByAI()
        //{
        //    if (isPlayerFallen) return;

        //    Debug.Log("PLAYER HIT BY AI — FALLING!");
        ...
        //    // Stop hammer + running animations
        //    _3rdPersonCharacterAnimator.SetBool("Run", false);

        //    // Play FALL animation
        //    _3rdPersonCharacterAnimator.SetTrigger("Fall");

        //    // StandUp after delay
        //    Invoke(nameof(StandUpAfterAIHit), 2f);
        //}

        //private void StandUpAfterAIHit()
        //{
        //    _3rdPersonCharacterAnimator.SetTrigger("StandUp");

        //    Invoke(nameof(ResumePlayerControl), 0.3f);
        //}
        ...
        //}
        // CALL THIS WHEN AI HITS PLAYER


        //public void GetHitByAI()
        //{
        //    Debug.Log("AI HIT PLAYER → RESPAWN!");

        //    // pause all control
        //    controllerPaused = true;

        //    // instantly teleport to respawn spot
        //    if (respawnPoint != null)
        //    {
        //        p_Rigidbody.position = respawnPoint.position;
        //        p_Rigidbody.velocity = Vector3.zero; // prevent drifting/sliding
        //    }
        //    // small delay then enable control again
        //    Invoke(nameof(EnablePlayerAfterRespawn), respawnFreezeTime);
        //}

        //private void EnablePlayerAfterRespawn()
        //{
        //    controllerPaused = false;
        //}
        ...
        public void GetHitByAI()
        {
            ...
            Debug.Log("AI HIT PLAYER → FLYING TO BASE!");
            ...
            // Use same respawn point
            flyTarget = respawnPoint;
        ...
        void RotateView(Vector2 yawPitchInput, float inputSensitivity, float cameraWeight) {
            ...
            switch (viewInputMethods) {
                ...
                case ViewInputModes.Retro: {
                        yawPitchInput = Vector2.up * (ControlFreak2.CF2Input.GetAxis("Horizontal") * ((mouseInputInversion == MouseInputInversionModes.Y || mouseInputInversion == MouseInputInversionModes.Both) ? -1 : 1));
    ...
    public class SuperFPEditor : Editor {
        ...
        public override void OnInspectorGUI() {
            ...
            EditorGUILayout.EndFoldoutHeaderGroup();
            // ----------------------------
            // Respawn Settings (Custom)
            // ----------------------------
            // ----------------------------
            // Respawn Settings (Custom)
            // ----------------------------
            EditorGUILayout.Space();
            GUILayout.Label("Respawn Settings", labelHeaderStyle, GUILayout.ExpandWidth(true));
            EditorGUILayout.Space();
            EditorGUILayout.BeginVertical(BoxPanel);

            t.respawnPoint = (Transform)EditorGUILayout.ObjectField(
                new GUIContent("Respawn Point", "Player will teleport here when AI hits"),
                t.respawnPoint,
                typeof(Transform),
                true
            );
        ...
        t.interactKey = (Key)EditorGUILayout.EnumPopup(new GUIContent("Interact Key", "The keyboard key used to Interact with objects that implement IInteract"),t.interactKey);
            ...
            #endregion
            // ----------------------
            // Hammer Settings (Custom)
            // ----------------------
            EditorGUILayout.Space();
            EditorGUILayout.LabelField("", GUI.skin.horizontalSlider, GUILayout.MaxHeight(6));
            EditorGUILayout.Space();

            GUILayout.Label("Hammer Settings", labelHeaderStyle, GUILayout.ExpandWidth(true));
            EditorGUILayout.Space();

            EditorGUILayout.BeginVertical(BoxPanel);

            t.hammer3D = (GameObject)EditorGUILayout.ObjectField(
                new GUIContent("Hammer Object", "Reference to hammer 3D object"),
                t.hammer3D,
                typeof(GameObject),
                true
            );

            t.hammerHideDelay = EditorGUILayout.FloatField(
                new GUIContent("Hammer Hide Delay", "Delay after which hammer disappears"),
                t.hammerHideDelay
            );
            ...
            if (GUI.changed)
            {
                ...
                Undo.RecordObject(t, "Undo Hammer Settings changes");
