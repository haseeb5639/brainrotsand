...
public class GDPRConsentDialog : IGDPRDialog
{
    ...
    public GDPRConsentDialog AddStringSection(string identifier, string title, string description)
    {
        if (string.IsNullOrEmpty(identifier))
        {
            Debug.LogError("Error: 'GDPR.identifier' was empty!");
        ...
        return AddSection(new Section(null, title, identifier, true, null, null));
    }
